...
def start_get_haplotypes(chrom,strand,snp_bases,read_APA_dict,snp_file_dict):
    ...
    for snp_pos, snp_base in snp_bases.items():
        ...
        if res is not None:
            a1,a2,A1_count_l,A2_count_l,all_APA_type,snpID,maf = res
            res_l.append([a1,a2,A1_count_l,A2_count_l,all_APA_type,snpID,maf,snp_pos])
    if len(res_l) != 0:
        return res_l
    else:
        return None
...
def get_haplotypes(readid_base_APA,snp_file_dict_res,strand):
    ...
    return a1,a2,A1_count_l,A2_count_l,all_APA_type,snpID,maf

# fisher
def simulate_fisher_exact_se(A1_apa1, A2_apa1, A1_apa2, A2_apa2,min_cov):
    if (A1_apa1+A1_apa2 >= min_cov) & (A2_apa1+A2_apa2 >= min_cov):
        observed = np.array([[A1_apa1, A2_apa1], [A1_apa2, A2_apa2]])
        if 0 in [A1_apa1, A2_apa1, A1_apa2, A2_apa2]:
            odds_ratio = ((A1_apa1 + 0.75) * (A2_apa2 + 0.75)) / ((A2_apa1 + 0.75) * (A1_apa2 + 0.75))
...
def apply_simulate_fisher(row,min_cov):
    return simulate_fisher_exact_se(row['A1_apa1'], row['A2_apa1'], row['A1_apa2'], row['A2_apa2'],min_cov)
