...
def start_get_haplotypes(chrom,strand,snp_bases,read_promoter_dict,snp_file_dict):
    ...
    for snp_pos, snp_base in snp_bases.items():
        ...
        if res is not None:
            a1,a2,A1_count_l,A2_count_l,all_promoter_type,snpID,maf = res
            res_l.append([a1,a2,A1_count_l,A2_count_l,all_promoter_type,snpID,maf,snp_pos])
    if len(res_l) != 0:
        return res_l
    else:
        return None
...
def get_haplotypes(readid_base_promoter,snp_file_dict_res,strand):
    ...
    return a1,a2,A1_count_l,A2_count_l,all_promoter_type,snpID,maf
...
def simulate_fisher_exact_se(A1_promoter1, A2_promoter1, A1_promoter2, A2_promoter2,min_cov):
    if (A1_promoter1+A1_promoter2 >= min_cov) & (A2_promoter1+A2_promoter2 >= min_cov):
        ...
        if 0 in [A1_promoter1, A2_promoter1, A1_promoter2, A2_promoter2]:
            odds_ratio = ((A1_promoter1 + 0.75) * (A2_promoter2 + 0.75)) / ((A2_promoter1 + 0.75) * (A1_promoter2 + 0.75))
