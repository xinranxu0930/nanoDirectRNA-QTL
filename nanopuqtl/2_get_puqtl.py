...
def count_haplotype(chrom, end, strand, bamfile, read_promoter_dict, snp_file_dict,threadsnum,base_minQ,read_minQ):
    ...
    haplotype_df = pd.DataFrame(columns=[
        ...
        "rsID","A1","A2",
        "A1_promoter_l","A2_promoter_l",
        "promoter_id_l",
        "EAF",
        ])
    result = start_get_haplotypes(chrom, strand, snp_bases, read_promoter_dict, snp_file_dict)
    if result is None:
...
def start_get_haplotypes(chrom,strand,snp_bases,read_promoter_dict,snp_file_dict):
    res_l = [] # 每一个元素都表示一个snp的结果 [snp_genome_0base,ref,alt,ref_l,alt_l,promoter_l]
    # snp_bases [pos0]:{read1:"A",read2:"A",read3:"T"}
    # read_promoter_dict read:promoter
    for snp_pos, snp_base in snp_bases.items():
        readid_base_promoter = [] # 每一个新的snp都有一个新的readid_base_promoter，里面是覆盖这个snp的所有read的base和promoter
        for read,base in snp_base.items():
            if read not in read_promoter_dict:
                continue
            else:
                readid_base_promoter.append([base,read_promoter_dict[read]]) # 这里是某个snp处的read、base和promoter的对应情况，因为不需要知道具体的read是什么，只要知道base和promoter的对应关系就可以，所以放在list中
        res = get_haplotypes(readid_base_promoter, snp_file_dict[f"{chrom}_{snp_pos}"],strand)
        if res is not None:
...
def get_haplotypes(readid_base_promoter,snp_file_dict_res,strand):
    if len(readid_base_promoter) == 0:
        return None
    snpID = snp_file_dict_res.split(";")[0]
    eaf = float(snp_file_dict_res.split(";")[3])
    all_base_list = list(i[0] for i in readid_base_promoter)
    all_promoter_type = list(set(i[1] for i in readid_base_promoter))
    ...
    if (a1 == "") and (a2 == ""):
        return None
    # 遍历all_promoter_type列表
    A1_count_l, A2_count_l = [], []
    if a1 != "" and a2 != "":
        A1_count_l = [sum(1 for base_promoter in readid_base_promoter if base_promoter[0] == a1 and base_promoter[1] == promoter) for promoter in all_promoter_type]
        A2_count_l = [sum(1 for base_promoter in readid_base_promoter if base_promoter[0] == a2 and base_promoter[1] == promoter) for promoter in all_promoter_type]
    elif a2 == "":
        A1_count_l = [sum(1 for base_promoter in readid_base_promoter if base_promoter[0] == a1 and base_promoter[1] == promoter) for promoter in all_promoter_type]
        A2_count_l = [0 for _ in range(len(all_promoter_type))]
    else:
        A2_count_l = [sum(1 for base_promoter in readid_base_promoter if base_promoter[0] == a2 and base_promoter[1] == promoter) for promoter in all_promoter_type]
        A1_count_l = [0 for _ in range(len(all_promoter_type))]
    return a1,a2,A1_count_l,A2_count_l,all_promoter_type,snpID,eaf

# fisher
def simulate_fisher_exact_se(A1_promoter1, A2_promoter1, A1_promoter2, A2_promoter2,min_cov):
    p_value = None
    beta = None
    se_log_or = None
    if (A1_promoter1+A1_promoter2 >= min_cov) & (A2_promoter1+A2_promoter2 >= min_cov):
        A1_promoter1 += 0.75 if A1_promoter1 == 0 else 0
        A2_promoter1 += 0.75 if A2_promoter1 == 0 else 0
        A1_promoter2 += 0.75 if A1_promoter2 == 0 else 0
        A2_promoter2 += 0.75 if A2_promoter2 == 0 else 0
        observed = np.array([[A1_promoter1, A2_promoter1], [A1_promoter2, A2_promoter2]])
        odds_ratio = (A1_promoter1 * A2_promoter2) / (A2_promoter1 * A1_promoter2)
        se_log_or = np.sqrt(1/A1_promoter1 + 1/A2_promoter2 + 1/A2_promoter1 + 1/A1_promoter2)
...
def apply_simulate_fisher(row,min_cov):
    return simulate_fisher_exact_se(row['A1_promoter1'], row['A2_promoter1'], row['A1_promoter2'], row['A2_promoter2'],min_cov)
...
if __name__ == "__main__":
    ...
    if len(haplotype_df) != 0:
        haplotype_df = haplotype_df[haplotype_df['promoter_id_l'].str.len() != 1] # 删除df['promoter_id_l']中长度为1的行
        ...
        for i in df.index:
            if len(df.loc[i, "promoter_id_l"])>2:
                subset_df = pd.DataFrame([df.loc[i, "promoter_id_l"], df.loc[i, "A1_promoter_l"], df.loc[i, "A2_promoter_l"]])
                ...
                df.loc[i, "A1_promoter1"] = grouped_df.iloc[0][max_sum_column]
                df.loc[i, "A2_promoter1"] = grouped_df.iloc[1][max_sum_column]
                df.loc[i, "A1_promoter2"] = grouped_df.iloc[0]["other"]
                df.loc[i, "A2_promoter2"] = grouped_df.iloc[1]["other"]
                df.loc[i, "promoter_id_l"] = f'{max_sum_column},other'
            else:
                df.loc[i, "A1_promoter1"] = df.loc[i, "A1_promoter_l"][0]
                df.loc[i, "A2_promoter1"] = df.loc[i, "A2_promoter_l"][0]
                df.loc[i, "A1_promoter2"] = df.loc[i, "A1_promoter_l"][1]
                df.loc[i, "A2_promoter2"] = df.loc[i, "A2_promoter_l"][1]
                df.loc[i, "promoter_id_l"] = ','.join(df.loc[i, "promoter_id_l"])
        del df['A1_promoter_l'], df['A2_promoter_l']
