...
def get_cov(chrom,strand,bam_file, df, pre,min_qscore):
    ...
    df['methy_rate'] = np.where(df['cov'] == 0, 0, (df['methy_num'] / df['cov'] * 100).round(2))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q", "--min_qscore", type=int, default=5, help="m6A min qscore(default=5)")
    ...
    pileup_file_res = f"{args.output_prefix}_m6A_sites_{args.chrom}_{args.strand}_tmp.csv"
    pileup_file_dict = f"{args.output_prefix}_m6A_read_{args.chrom}_{args.strand}_tmp.pkl"
    ...
    pileup_df = pileup_df[pileup_df['methy_rate'] >= 10]
