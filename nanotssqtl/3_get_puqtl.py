...
def count_haplotype(chrom, end, strand, bamfile, read_TSS_dict, snp_file_dict,threadsnum,base_minQ,read_minQ):
    # read_TSS_dict read对应的TSS read:TSS
    ...
    result = start_get_haplotypes(chrom, strand, snp_bases, read_TSS_dict, snp_file_dict)
    if result is None:
...
def start_get_haplotypes(chrom,strand,snp_bases,read_TSS_dict,snp_file_dict):
    res_l = [] # 每一个元素都表示一个snp的结果 [snp_genome_0base,ref,alt,ref_l,alt_l,TSS_l]
    # snp_bases [pos0]:{read1:"A",read2:"A",read3:"T"}
    # read_TSS_dict read:TSS
    for snp_pos, snp_base in snp_bases.items():
        readid_base_TSS = [] # 每一个新的snp都有一个新的readid_base_TSS，里面是覆盖这个snp的所有read的base和TSS
        for read,base in snp_base.items():
            if read not in read_TSS_dict:
                continue
            else:
                readid_base_TSS.append([base,read_TSS_dict[read]]) # 这里是某个snp处的read、base和TSS的对应情况，因为不需要知道具体的read是什么，只要知道base和TSS的对应关系就可以，所以放在list中
        res = get_haplotypes(readid_base_TSS, snp_file_dict[f"{chrom}_{snp_pos}"],strand)
        if res is not None:
            a1,a2,A1_count_l,A2_count_l,all_TSS_type,snpID,maf = res
            res_l.append([a1,a2,A1_count_l,A2_count_l,all_TSS_type,snpID,maf,snp_pos])
    if len(res_l) != 0:
        return res_l
    else:
        return None

def get_haplotypes(readid_base_TSS,snp_file_dict_res,strand):
    if len(readid_base_TSS) == 0:
        return None
    snpID = snp_file_dict_res.split(";")[0]
    maf = float(snp_file_dict_res.split(";")[3])
    all_base_list = list(i[0] for i in readid_base_TSS)
    all_TSS_type = list(set(i[1] for i in readid_base_TSS))
    ...
    if a1 != "" and a2 != "":
        A1_count_l = [sum(1 for base_TSS in readid_base_TSS if base_TSS[0] == a1 and base_TSS[1] == TSS) for TSS in all_TSS_type]
        A2_count_l = [sum(1 for base_TSS in readid_base_TSS if base_TSS[0] == a2 and base_TSS[1] == TSS) for TSS in all_TSS_type]
    elif a2 == "":
        A1_count_l = [sum(1 for base_TSS in readid_base_TSS if base_TSS[0] == a1 and base_TSS[1] == TSS) for TSS in all_TSS_type]
        A2_count_l = [0 for _ in range(len(all_TSS_type))]
    else:
        A2_count_l = [sum(1 for base_TSS in readid_base_TSS if base_TSS[0] == a2 and base_TSS[1] == TSS) for TSS in all_TSS_type]
        A1_count_l = [0 for _ in range(len(all_TSS_type))]
    return a1,a2,A1_count_l,A2_count_l,all_TSS_type,snpID,maf
...
def simulate_fisher_exact_se(A1_TSS1, A2_TSS1, A1_TSS2, A2_TSS2,min_cov):
    if (A1_TSS1+A1_TSS2>= min_cov) & (A2_TSS1+A2_TSS2>= min_cov):
        ...
        if 0 in [A1_TSS1, A2_TSS1, A1_TSS2, A2_TSS2]:
            odds_ratio = ((A1_TSS1 + 0.75) * (A2_TSS2 + 0.75)) / ((A2_TSS1 + 0.75) * (A1_TSS2 + 0.75))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Nanopore direct RNA data call tuQTL.')
    ...
    parser.add_argument("-f","--read_overlap_file", type=str, help="read2FANTOM overlap file path")
    ...
    overlap_df = pd.read_csv(args.read_overlap_file, sep="\t",header=None, names=["readid","tss_name"],usecols=[3,9])
    df_dict = dict(zip(overlap_df["readid"], overlap_df["tss_name"])) # readid:tss_name
    ...
    if len(haplotype_df) != 0:
        haplotype_df = haplotype_df[haplotype_df['TSS_id_l'].str.len() != 1] # 删除df['TSS_id_l']中长度为1的行
        ...
        for i in df.index:
            if len(df.loc[i, "TSS_id_l"])>2:
                subset_df = pd.DataFrame([df.loc[i, "TSS_id_l"], df.loc[i, "A1_TSS_l"], df.loc[i, "A2_TSS_l"]])
                ...
                df.loc[i, "A1_TSS1"] = grouped_df.iloc[0][max_sum_column]
                df.loc[i, "A2_TSS1"] = grouped_df.iloc[1][max_sum_column]
                df.loc[i, "A1_TSS2"] = grouped_df.iloc[0]["other"]
                df.loc[i, "A2_TSS2"] = grouped_df.iloc[1]["other"]
                df.loc[i, "TSS_id_l"] = f'{max_sum_column},other'
            else:
                df.loc[i, "A1_TSS1"] = df.loc[i, "A1_TSS_l"][0]
                df.loc[i, "A2_TSS1"] = df.loc[i, "A2_TSS_l"][0]
                df.loc[i, "A1_TSS2"] = df.loc[i, "A1_TSS_l"][1]
